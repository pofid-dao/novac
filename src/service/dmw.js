...
class Dmw {
  constructor() {
    ...
    this.callContract = serojs.callContract(config.dmw.abi, config.dmw.address);
  }
  ...
  async getMinBackedAmount(_backedCoin) {
    const act = account.getCurrent();
  ...
  async executeMethod(
    ...
    let convertAddress = config.dmw.address;
    ...
    executeData.gas = gas;
    let contractAddress = config.dmw.address;
    if (proxy == true && _method == 'issue') {
